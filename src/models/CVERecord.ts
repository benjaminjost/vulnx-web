export class CVERecord {
  ...
  constructor({
    ...
    this.publishedAt = cve_created_at
      ? new Date(cve_created_at as string)
      : new Date();
    this.updatedAt = cve_updated_at
      ? new Date(cve_updated_at as string)
      : new Date();

    const affectedProductsArray = Array.isArray(affected_products)
      ? affected_products
      : [];
    const firstProduct =
      affectedProductsArray.length > 0
        ? (affectedProductsArray[0] as Record<string, unknown>)
        : null;
    this.product =
      firstProduct && typeof firstProduct.product === "string"
        ? firstProduct.product
        : null;
    this.vendor =
      firstProduct && typeof firstProduct.vendor === "string"
        ? firstProduct.vendor
        : null;
    ...
    this.isPatchAvailable =
      typeof is_patch_available === "boolean" ? is_patch_available : false;
    this.vulnerabilityType =
      typeof vulnerability_type === "string" ? vulnerability_type : null;
